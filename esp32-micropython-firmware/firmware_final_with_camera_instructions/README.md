# ESP32/ESP32-S3 MicroPython 固件包（含相机支持）

## 📦 固件包内容

本目录包含完整的ESP32和ESP32-S3 MicroPython固件，包括**相机模块支持**的构建脚本和说明。
...
### 📷 ESP32-S3相机固件（需要构建）
- `build_camera_firmware.sh` - 相机固件构建脚本 ⭐
- `flash_esp32s3_camera.sh` - 相机固件烧录脚本 ⭐
- `esp32s3_camera_*.bin` - 相机固件文件（构建后生成）
